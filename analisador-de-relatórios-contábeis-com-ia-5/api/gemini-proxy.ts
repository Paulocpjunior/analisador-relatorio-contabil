...
// --- Tipos duplicados de 'types.ts' para uso no ambiente serverless ---
// É necessário duplicar pois não podemos importar diretamente de `../types` em uma função serverless simples.
...
interface AnalysisHistoryItem {
  ...
  fileName: string;
  analysisDate: string;
  result: any; // Simplified for backend
  employeeName?: string;
  companyName?: string;
}
...
// --- Schemas e Prompts (copiados de 'services/geminiService.ts' original) ---
...
6.  **Análises Customizadas (customAnalysisResults)**: O usuário não forneceu nenhuma solicitação customizada. Retorne um array vazio.`;
    ...
    return JSON.parse(response.text.trim().replace(/^```json\s*|```\s*$/g, ''));
}
